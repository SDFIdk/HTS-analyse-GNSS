...
if (!isnan(Beta0))
  h = plot(ax1,t,x,'*k;GNSS observation;',...
     ...
     sx.t2,sx.x2+sx.prediction,['--g;' num2str(100 - alpha.*100) '% Prediction interval for Predicted GNSS observation;'],...
     sx.t2,sx.x2-sx.prediction,'--g;;',...
     sx.t2,sx.x2+sx.conf_mean_response_known,['-b;' num2str(100 - alpha.*100) '% Confidence interval for Estimated True Ellipsoidal Height;'],...
...
textstring = [textstring "Estimated standard deviation on data from all measurements "...
